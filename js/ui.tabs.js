...
;(function(jQuery) {
	...
	jQuery.widget("ui.tabs", {
		init: function() {
			...
			jQuery(this.element).bind('setData.tabs', function(event, key, value) {
		...
		tabify: function(init) {
			...
			this.$tabs.each(function(i, a) {
				...
				// remote tab
					...
					var $panel = jQuery('#' + id);
					if (!$panel.length) {
						$panel = jQuery(o.panelTemplate).attr('id', id).addClass(o.panelClass)
			...
			if (init) {
				...
				jQuery(this.element).hasClass(o.navClass) || jQuery(this.element).addClass(o.navClass);
					...
					if (location.hash) {
						if (a.hash == location.hash) {
							...
							if (jQuery.browser.msie || jQuery.browser.opera) {
								var $toShow = jQuery(location.hash), toShowId = $toShow.attr('id');
								$toShow.attr('id', '');
								setTimeout(function() {
				...
				if (o.selected !== null) {
                    ...
                    // load if remote tab
    				if (jQuery.data(this.$tabs[o.selected], 'load.tabs'))
				...
				o.disabled = jQuery.unique(o.disabled.concat(
					jQuery.map(this.$lis.filter('.' + o.disabledClass),
			...
			this.$tabs.unbind('.tabs').bind(o.event, function() {
				...
				if (($li.hasClass(o.selectedClass) && !o.unselect)
					...
					|| jQuery(self.element).triggerHandler('tabsselect', [self.ui(this, $show[0])], o.select) === false
					) {
					this.blur();
					return false;
				}
		...
		add: function(url, label, index) {
			...
			var o = this.options;
			var $li = jQuery(o.tabTemplate.replace(/#\{href\}/, url).replace(/#\{label\}/, label));
			$li.data('destroy.tabs', true);

			var id = url.indexOf('#') == 0 ? url.replace('#', '') : this.tabId( jQuery('a:first-child', $li)[0] );

			// try to find an existing element before creating a new one
			var $panel = jQuery('#' + id);
			if (!$panel.length) {
				$panel = jQuery(o.panelTemplate).attr('id', id)
			...
			if (this.$tabs.length == 1) {
				...
				var href = jQuery.data(this.$tabs[0], 'load.tabs');
				if (href)
					this.load(index, href);
			}

			// callback
			jQuery(this.element).triggerHandler('tabsadd',
		...
		remove: function(index) {
			...
			// callback
			jQuery(this.element).triggerHandler('tabsremove',
		...
		enable: function(index) {
			...
			// callback
			jQuery(this.element).triggerHandler('tabsenable',
		...
		disable: function(index) {
			...
			if (index != o.selected) { // cannot disable already selected tab
				...
				// callback
				jQuery(this.element).triggerHandler('tabsdisable',
		...
		load: function(index, callback) { // callback is for internal usage only
			...
			if (!url || (jQuery.data(a, 'cache.tabs') && !bypassCache)) {
				callback();
				return;
			}
		...
		destroy: function() {
			var o = this.options;
			jQuery(this.element).unbind('.tabs')
				.removeClass(o.navClass).removeData('tabs');
			this.$tabs.each(function() {
				var href = jQuery.data(this, 'href.tabs');
				if (href)
					this.href = href;
				var $this = jQuery(this).unbind('.tabs');
